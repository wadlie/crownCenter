...
export function updateForm(code, formname, data) {
  return new Promise(function(resolve, reject){
      Axios.put('http://' + IP + ':' + port + '/api/formsubmission/' + formname + '/update/' + code, data, config)
        .then(function(response){
...
export function getFieldHistory(farmcode) {
  return new Promise(function(resolve, reject){
    Axios.get('http://' + IP + ':' + port + '/api/formfieldhistory/farmcode/' + farmcode, config)
      .then(function(response){
        console.log('Field history for ' + farmcode + ' exists in the database!');
        resolve(response.data);
      }).catch(function(error) {
        console.log('Field history for ' + farmcode + ' does not exist in the database!');
        resolve(null);
      })
  })
}
