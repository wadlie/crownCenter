...
const RequiredQuestions = ['barcode', 'gps_coordinates'];
  ...
  .then(function(exists) {
    if (exists) {
      ...
      database.updateForm(FormData.BagData.barcode, 'FormBag', FormData.BagData)
        ...
        .catch((error) => database.errorHandling(error, 'bag gps', ErrorLog).then(app.result(ErrorLog)));
    } else {
...
function initializeFields() {
  return new Promise(function(resolve, reject){
    ...
    const [lat, lon] = app.getAnswer('gps_coordinates').split(';');
