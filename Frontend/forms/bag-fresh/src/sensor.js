...
const RequiredQuestions = ['barcode', 'in_field_bag_fresh_weight'];
  ...
  .then(function(exists) {
    if (exists) {
      ...
      database.updateForm(FormData.BagData.barcode, 'FormBag', FormData.BagData)
        ...
        .catch((error) => database.errorHandling(error, 'bag fresh', ErrorLog).then(app.result(ErrorLog)));
    } else {
...
function initializeFields() {
  return new Promise(function(resolve, reject){
    ...
    FormData.BagData.in_field_bag_fresh_weight = app.getAnswer('in_field_bag_fresh_weight');
    if(app.isAnswered('notes')) {
      FormData.BagData.notes = app.getAnswer('notes');
    }
    resolve();
  })
}